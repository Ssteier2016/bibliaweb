...
function App() {
  ...
  const [prayerModal, setPrayerModal] = useState({ visible: false, verse: null }); // Nuevo
  ...
  useEffect(() => {
    const handleClickOutside = (e) => {
      if (contextMenuRef.current && !contextMenuRef.current.contains(e.target)) {
        ...
        setCommentSubmenu(false);
        setConcordanceSubmenu(false);
        setPrayerModal({ visible: false, verse: null }); // Cierra modal de oración
      }
    };
                    ...
                    >
                      ...
                      {prayerModal.visible && prayerModal.verse?.verse === verse.verse && (
                        <div className="prayer-modal">
                          {/* Se implementará en BibleReading.js */}
                        </div>
                      )}
                    </div>
                  );
